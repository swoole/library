...
* The mysqli proxies' reconnect logic never ran under the default mysqli report mode of PHP 8.1+, where a failure is thrown as `\mysqli_sql_exception` instead of returned as `false`; a lost connection is now reconnected in that mode, too. The proxies' list of methods that may reconnect is also anchored properly, where it used to match any method name containing one of the listed names, and `mysqli::execute_query()` (PHP 8.2+) is now listed explicitly instead of matching by accident. A lost connection inside a transaction, one started with `begin_transaction()` or running with `autocommit(false)`, is reported to the caller instead of being hidden behind a reconnect that would have committed only part of the transaction; `\Swoole\Database\MysqliProxy::inTransaction()` tells whether the connection is in one.
...
* `\Swoole\Database\PDOStatementProxy` did not record that it had reconnected its parent, so a second lost connection on the same statement was not recovered: the statement was prepared again on the dead connection and the retried `execute()` failed.
